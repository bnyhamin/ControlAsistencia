...
console.info(window.opener.com.atento.gap.ValidarIncidencia);
console.info(window.opener.com.atento.gap.ValidarIncidencia.testing);
console.info(window.opener.com.atento.gap.ValidarIncidencia.testing());*/
...
if("<?php echo $punto;?>"=="1"){
    if("<?php echo $mensaje;?>"=="OK"){
        ...
        //window.opener.document.forms['frm'].submit();
        //-->
        var resp_cod="<?php echo $responsable_codigo;?>";
        var emp_cod="<?php echo $empleado_codigo;?>";
        var asi_cod="<?php echo $asistencia_codigo;?>";
        var dest=window.opener.document.getElementById('hdd_registro');
        var tiempo_efectivo=window.opener.document.getElementById('hdd_tiempo_efectivo').value;
        var extension_tiempo=window.opener.document.getElementById('hdd_extension_tiempo').value;
        dest.value=resp_cod+'_'+emp_cod+'_'+asi_cod+'_'+tiempo_efectivo+'_'+extension_tiempo;
        //-->
        window.opener.document.frm.cmdx.click();
        window.close();
...
if(c!=0){	
	...
	if(hh_dd==0){ //diario
	    if(codigo==11) {
	  	...
	  	if(hh_dd==1)
		{ //horario		
			if(codigo==42 || codigo==43 || codigo==79 || codigo==166) 
			{
				setear_tiempo(0);
				activar_tiempo(true);
			}
