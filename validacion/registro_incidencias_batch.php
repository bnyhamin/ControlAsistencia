...
if (isset($_POST['hddaccion'])){
  if ($_POST['hddaccion']=='INS'){
	...
	//mcortezc21052015
        if($incidencia_codigo == 181){
            $respuesta = $o->existe_incidencia(154);
            if(!$respuesta){
                $mensaje = "¡Error! Solo se puede registrar cuando exista incidencia de SALIDA ANTICIPADA ANTES DE FIN DE TURNO CON DESCUENTO Y CARTA.'";
                $flag_registrar = 0;
            }
        }
        ...
        ?>
			<script type="text/javascript">
				alert('Se ejecuto registro, verifique resultados.');
				window.close();
			</script>
	<?php
