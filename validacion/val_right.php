...
if (isset($_GET["hddemp"])) $hddemp = $_GET["hddemp"];
if (isset($_GET["hdddni"])) $hdddni = $_GET["hdddni"];
if (isset($_GET["hddsup"])) $hddsup = $_GET["hddsup"];
...
if (isset($_POST['hdddeltx'])){
  if ($_POST['hdddeltx']=='DEL'){
		//-- eliminar tiempo
		$o->tiempo_codigo = $_GET['codigo'];
		$mensaje = $o->eliminar_tiempo();
		if($mensaje=='OK'){
		?>
		<script type="text/javascript">
                    window.parent.frames[1].location="val_left.php?f=<?php echo $fecha ?>&e=<?php echo $empleado_cod ?>";		 
...
window.parent.frames[1].location="val_left.php?f=<?php echo $fecha ?>&e=<?php echo $empleado_cod ?>&tab=<?php echo $tab ?>&hddemp=<?php echo $hddemp ?>&hdddni=<?php echo $hdddni ?>&hddsup=<?php echo $hddsup ?>&indica=<?php echo $indica ?>";
return false;
}
function cmdIncidencias_onclick(){
			...
			if (rpta != '' ) {	
			  if(ivac!=1){	
			     var arr=rpta.split('_');
                             //alert(arr[1]);
			     if(arr[1]==1)//window.showModalDialog("registro_incidencias.php?asistencia=" + arr[0] + "&responsable=" + responsable_codigo + "&empleado=" + empleado_codigo + "&empleado_nombres=" + empleado ,"Incidencias","dialogWidth:500px; dialogHeight:300px"); 
				  CenterWindow("registro_incidencias.php?asistencia=" + arr[0] + "&responsable=" + responsable_codigo + "&empleado=" + empleado_codigo + "&num=" + num + "&fecha=" + fecha + "&area=" + area_responsable,"ModalChild",650,400,"yes","center");
                                  //window.open("registro_incidencias.php?asistencia=" + arr[0] + "&responsable=" + responsable_codigo + "&empleado=" + empleado_codigo + "&num=" + num + "&fecha=" + fecha + "&area=" + area_responsable);
...
function cmdTiempos_onclick(){
 ...
 /*
	 var num=document.frm.num.value; 
	 if(num!=0){ //registro de incidencias de asistencia
		CenterWindow("registro_tiempos.php?responsable=" + responsable_codigo + "&empleado=" + empleado_codigo + "&fecha=" + fecha + "&area=" + area_responsable,"ModalChild",650,400,"yes","center");
	  }else//registro de incidencias sin asistencia definida
		  alert('Debe tener una entrada de asistencia marcada!!');
  */
...
function cmdEliminar_tiempo_onclick(codigo){
if (confirm('Seguro de eliminar tiempo de conexión?')==false) return false;
			  document.frm.action +="?codigo=" + codigo;
			  document.frm.hdddeltx.value='DEL';
			  document.frm.submit();
...
case '2':	//Compartido	
        //var responsable_codigo="<?php echo $id ?>";
		//alert('sesion :' + responsable_codigo);
		//alert('mostrado:' + responsable);
        //if(responsable==responsable_codigo){
			  if(incidencia==0){
			   if (confirm('Seguro de eliminar al supervisor?')==false) return false;
				  document.frm.action +="?codigo=" + asistencia + "&responsable=" + responsable;
		...
		//}else{
		  //alert('Supervisor externo a su grupo!!, no lo puede eliminar');
		//}
		break;
	}
}
...
//	 $rpta=$emp->Query_Turno();
//	 if ($emp->turno_codigo==null or $emp->turno_codigo==''){
//	 	$rpta=$emp->Query();
//	 }
	 ...
	 //Verificar incidencia de vacaciones;
