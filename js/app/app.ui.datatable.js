...
(function($) {
	...
	$.fn.dataTable=function(_json){
		...
		return this.each(function(){
			...
			//_width_grid=_config.width+50;
			_total_rows=(_config.data.length);						
			//width
			_html+='<div class="datatable-content" style="width:'+_width_grid+'px;">';
			//_html+='<div class="datatable-content" style="width:'+_config.width+'px;">';

			//COLUMNS
				_html+='<div class="datatable-header">';
					_html+='<table style="width:'+_width_grid+'px;"><tr>';
					//autonumeric
					_html+='<th style="width:30px;border-right:1px solid #ced0cd;"><div style="text-align:center;">##</div></th>';
					for (var _pos = 0; _pos < _total_columns; _pos++){
				...
				_html += '<div style="overflow-y:auto;overflow-x:hidden;font-weight:normal;height:'+(_config.height || 100)+'px;" class="datatable-data"><table id="'+_config.idGrid+'" style="width:'+_width_grid+'px;">';					
				$.each(_config.data,function(key,row){
					$.addRowTmp(row);//add row a tmp data
					_html +='<tr>';
					//autonumeric
					_html += '<td style="width:30px;border-right:1px solid #ced0cd;"><div style="text-align:right;">'+(key + 1)+'</div></td>';
					_pos = 0;
...
//						
							...
							_html += '<table style="width:auto;"><tr>';
								if(typeof _config.buttons!='undefined' && typeof _config.buttons=='object'){
									$.each(_config.buttons,function(key,button){
										_html += '<td width="15"><img src="../../../agencia/views/app/'+(_config.path+button.ico)+'" alt="'+button.ico+'" title="'+button.title+'" id="'+_config.idGrid+'_button_'+key+'" /></td>';	
									});									
								}								
								//_html += '<td><div><img src="../../../agencia/views/app/'+_config.path+'refresh.png" alt="reload" title="RELOAD" id="'+_config.idGrid+'_refresh" /></div></td>';
								_html += '<td width="15"><img src="../../../agencia/views/app/'+_config.path+'first.png" alt="first" title="PRIMERO" id="'+_config.idGrid+'_first" /></td>';
								_html += '<td width="15"><img src="../../../agencia/views/app/'+_config.path+'previous.png" alt="previous" title="ANTERIOR" id="'+_config.idGrid+'_previous" /></td>';
								_html += '<td width="15"><img src="../../../agencia/views/app/'+_config.path+'next.png" alt="next" title="SIGUIENTE" id="'+_config.idGrid+'_next" /></td>';
								_html += '<td width="15"><img src="../../../agencia/views/app/'+_config.path+'last.png" alt="last" title="ULTIMO" id="'+_config.idGrid+'_last" /></td>';
	...
	$.createExtra=function(obj){
		switch (obj.type){
			...
			break;
			case 'img':
				return '<img src="../../../agencia/views/app/'+obj.ico+'" alt="'+obj.id+'" id="_img'+obj.id+'" />';
			break;
			case 'radio':
