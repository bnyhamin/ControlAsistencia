...
function Quitar_Turno_Especial(){
	...
	$cn = $this->getMyConexionADO();
        //$cn->debug=true;
	if($cn){
		$params = array($this->empleado_codigo_registro,$this->empleado_codigo );
        $ssql = " UPDATE ca_asistencia_especial SET ae_activo=0,usuario_modifica=?,fecha_modifica=getdate() ";
    ...
    if($rs->RecordCount() > 0){
        ...
        while(!$rs->EOF){
            ...
            if ($rs->fields[11]!=''){ //si tiene asignado 
                    $cadena .=$rs->fields[13]." (".$rs->fields[17]." hrs)";
            }else{
                    $cadena .="S/TE";
            }
            $cadena .="</td>";
            ...
            if ($rs->fields[11] !=''){ //si tiene asignado
                    $cadena .="<img border='0' src='../../Images/asistencia/delete_small.gif' onclick='Quitar(". $rs->fields[0] .")' style='cursor:hand' title='Quitar turno:".$rs->fields[0] . "'>";
            }else{
                    $cadena .="";
            }
            $cadena .="</td>";	
...
function Importar_Eventos_Tmp(){
	...
	if($cn){
	    $params = array($this->empleado_codigo_registro,$this->carga_codigo );
		$ssql ="exec spCA_Importar_eventos ?,?";
		$rs = $cn->Execute($ssql, $params);
	}
}
...
function Query_Turno_Feriados(){
	...
	if($cn){
		...
		$ssql .= " where convert(varchar(8),fecha_feriado,112) between " . $this->te_fecha_inicio . " and " . $this->te_fecha_fin." and Tipo_Feriado = 1 and Feriado_Activo =1"; 
	    $rs = $cn->Execute($ssql);
		//echo $ssql;
		if (!$rs->EOF){
			while(!$rs->EOF){
...
function Query_Rol_Admin(){
	...
	if($cn){
		...
		$ssql.= " and empleado_codigo=? and rol_codigo in (3,5)"; 
	    $rs = $cn->Execute($ssql, $params);
		if(!$rs->EOF){
			$rpta = "OK";
		}else{
...
// }
  ...
  function esAdministrador($empl_codigo){
    ...
    function Obtener_Tiempo_Horario($item_codigo){
        $cn = $this->getMyConexionADO();
        //$cn->debug = true;
        $params = array($item_codigo);
        $sql = "select item_default2 from Items where Item_Codigo = ?";
        $rs = $cn->Execute($sql, $params);
        if($rs->RecordCount() > 0){
            return $rs->fields[0] * 60;
        }else{
            return 0;
        }
    }
    
    function Actualizar_Movimiento_Cambio($tc_codigo, $usuario_asigna,$emp_mov_codigo){
        $cn = $this->getMyConexionADO();
        $params = array($tc_codigo, $usuario_asigna, $_SERVER["REMOTE_ADDR"],$emp_mov_codigo);
        $sql = "update Movimiento_cambios
                set tc_codigo = ?,
                	cc_usuario_asigna = ?,
                	cc_fecha_asigna = GETDATE(),
                	cc_ip_asigna = ?
    ...
    function Listar_Empleado_Vacaciones($area_codigo=0,$empleado_id=0,$controlar=0){
        $ssql="";
        $cn = $this->getMyConexionADO();
        //$cn->debug=true;
        $ssql=" Select  em.empleado as Empleado ";
        $ssql.=" ,convert(varchar(10),vs.fecha_inicio,103) as Desde ";
        $ssql.=" ,convert(varchar(10),vs.fecha_fin,103) as Hasta ";
        $ssql.=" ,vs.numero_dias as Dias ";
        $ssql.=" ,case when vs.aplica_adelanto='S' then 'Si' else 'No' end as Adel ";
        $ssql.=" ,vs.observaciones as Observaciones ";
        $ssql.=" ,es.Estado_Descripcion as Estado ";
        $ssql.=" from  vacaciones_solicitud vs ";
        $ssql.=" inner join vdatos em on vs.empleado_codigo=em.empleado_codigo ";
        $ssql.=" inner join estados es on vs.estado_codigo=es.estado_codigo ";
        $ssql.=" inner join areas a on em.area_codigo=a.area_codigo ";
        $ssql.=" left join vacaciones_campana vc on vs.vac_cam_codigo=vc.vac_cam_codigo ";
        
        if(intval($controlar)==1){
            
            $ssql.=" left join CA_Asignacion_Empleados cae on vs.Empleado_Codigo = cae.Empleado_Codigo ";
            $ssql.=" and cae.Asignacion_Activo = 1 ";
            
        }
        $ssql.=" where  vs.estado_codigo in (1,25) ";
        $ssql.=" and (vs.fecha_inicio>getdate()-1 or (getdate() between vs.fecha_inicio and vs.fecha_fin )) ";
        
        if(intval($controlar)==1){
            
            $ssql.=" and a.empleado_responsable= (select empleado_responsable from Areas where Area_Codigo = ".$area_codigo." ) ";
            //$ssql.=" and cae.Asignacion_Activo = 1 ";
            $ssql.=" and 1=  case when not exists(select empleado_responsable  from Areas  where Area_Codigo = ".$area_codigo."  and empleado_responsable=".$empleado_id.") ";
            $ssql.=" then  case when cae.Responsable_Codigo=".$empleado_id."  then 1  else 0  end  else 1  end ";
            
        }
        
        $ssql.=" Order by es.estado_codigo desc,vs.vac_sol_codigo DESC ";
    ...
    function Listar_Empleado_Movimientos($area_codigo=0,$empleado_id=0,$controlar=0){
        
        $cn = $this->getMyConexionADO();
        
        $ssql="";
        $ssql=" select dbo.UDF_DNI(em.Empleado_Codigo) as DNI ";
        $ssql.=" ,dbo.UDF_EMPLEADO_NOMBRE(em.Empleado_Codigo) as Empleado ";
        $ssql.=" ,a.Area_Descripcion as Area ";
        $ssql.=" ,m.Movimiento_Descripcion ";
        $ssql.=" ,convert(varchar,em.Emp_Mov_Fecha_Inicio,103) as Desde ";
        $ssql.=" ,convert(varchar,em.Emp_Mov_Fecha_Fin,103) as Hasta ";
        $ssql.=" ,e.Estado_descripcion as Estado ";
        $ssql.=" from Empleado_Movimiento em ";
        $from.= " inner join empleados on em.Empleado_Codigo = empleados.empleado_codigo and  empleados.Estado_Codigo = 1 ";
        
              $ssql.=" inner join movimiento m on em.Movimiento_codigo = m.movimiento_codigo ";
              $ssql.=" inner join Estados e on em.Estado_codigo = e.Estado_codigo ";
              $ssql.=" inner join Empleado_Area ea on em.Empleado_Codigo = ea.Empleado_Codigo and ea.Empleado_Area_Activo = 1 ";
              $ssql.=" inner join areas a on a.Area_Codigo = ea.Area_Codigo ";
              
              if(intval($controlar)==1){
                $ssql.=" left join CA_Asignacion_Empleados cae on em.Empleado_Codigo = cae.Empleado_Codigo ";
                    $ssql.=" and cae.Asignacion_Activo = 1 ";
                    
              }
        $ssql.=" where em.Estado_codigo in (1,2) ";
              $ssql.=" and (em.Emp_Mov_Fecha_Inicio>getdate()-1 or (getdate() between em.Emp_Mov_Fecha_Inicio and em.Emp_Mov_Fecha_Fin )) ";
              
              if(intval($controlar)==1){
                  
                  $ssql.=" and a.empleado_responsable = (select empleado_responsable from Areas where Area_Codigo = ".$area_codigo." ) ";
                $ssql.=" and 1= ";
                   $ssql.=" case when not exists(select empleado_responsable ";
                      $ssql.=" from Areas  where Area_Codigo = ".$area_codigo." ";
                      $ssql.=" and empleado_responsable=".$empleado_id.") ";
                   $ssql.=" then ";
                      $ssql.=" case when cae.Responsable_Codigo=".$empleado_id." ";
                      $ssql.=" then 1 ";
                      $ssql.=" else 0 ";
                      $ssql.=" end ";
                   $ssql.=" else 1 ";
        ...
        $ssql.=" and em.movimiento_codigo in (5,6,8,9,14,36) ";
        $ssql.=" order by Area,Empleado,em.Emp_Mov_Fecha_Inicio ";
    ...
    function Listar_Empleado_Modalidad($bandeja=0,$area_codigo=0,$empleado_id=0,$controlar=0){
        $cn = $this->getMyConexionADO();
        //echo "xxx".$bandeja;
        $sql = "Select rtrim(ltrim(str(e.Empleado_Codigo))) + '-' + rtrim(ltrim(str(mc.Emp_Mov_codigo))) as root,
                    e.Empleado_Dni as DNI,
                    e.Empleado_Apellido_Paterno + ' ' + e.Empleado_Apellido_Materno + ' ' + e.Empleado_Nombres as Empleado,
                    a.Area_Descripcion as area,CONVERT(varchar(10), em.emp_mov_fecha_inicio,103) as fecha_inicio,
                    modalidad.Item_Descripcion as modalidad,
                    horario.Item_Descripcion as horario,
                    estados.Estado_descripcion as estado 
                from Empleado_Movimiento em 
                    ...
                    if($bandeja==1){
                        if(intval($controlar)==1){
                            $sql .= " left join CA_Asignacion_Empleados cae on em.Empleado_Codigo = cae.Empleado_Codigo ";
                            $sql .= " and cae.Asignacion_Activo = 1 ";
                        }
                    }
                    
                    $sql .= " where em.Movimiento_codigo = 28 ";
                        $sql .= " and em.Estado_codigo in (1,6) ";
                    
                    if($bandeja==1){
                        
                        if(intval($controlar)==1){
                        
                            $sql .= "and a.empleado_responsable = (select empleado_responsable from Areas where Area_Codigo = ".$area_codigo." ) ";
                            $sql .= " and 1= ";
                               $sql .= "case when not exists(select empleado_responsable ";
                                  $sql .= "from Areas  where Area_Codigo = ".$area_codigo." ";
                                  $sql .= "and empleado_responsable=".$empleado_id.") ";
                               $sql .= "then ";
                                  $sql .= "case when cae.Responsable_Codigo=".$empleado_id." ";
                                  $sql .= "then 1 ";
                                  $sql .= "else 0 ";
                                  $sql .= "end ";
                               $sql .= "else 1 ";
                    ...
                    $sql .= " and em.Emp_Mov_Fecha_Inicio > getdate() ";
    ...
    function Consulta_Empleado_Generacion($empleado_codigo){
        $cn = $this->getMyConexionADO();
        $params = array($empleado_codigo);
        $sql = "select empleado_apellido_paterno + ' ' + empleado_apellido_materno + ' ' + empleado_nombres as empleado
                from empleados 
