...
function listado_eventos_validador(){
    ...
    $ssql.=" ae.responsable_codigo as supervisor_codigo, ";
    $ssql.=" ( select es.empleado_apellido_paterno+' '+es.empleado_apellido_materno+' '+es.empleado_nombres ";
    $ssql.=" from empleados as es where es.empleado_codigo = ae.responsable_codigo ) as supervisor_descripcion ,ar.area_descripcion, ev.ee_codigo ,ev.num_ticket,loc.Local_Descripcion as local_descripcion,cas.area_codigo ";
    $ssql.=" from CA_Eventos ev inner join empleados e ";
    $ssql.=" on ev.empleado_codigo=e.empleado_codigo ";
    //SE AGREGO EL CAMPO LOCAL DESCRIPCION
    $ssql.=" left join Locales loc on loc.Local_Codigo = e.Local_Codigo ";
    ...
    if($this->area!="0") $ssql.=" and j.area_codigo in (".$this->area.")";
        ...
        while(!$rs->EOF){
            ...
            $area_codigo    = $rs->fields[14];
            $mostrar = $this->verifica_flujo_evento($p_empleado, $p_asistencia, $p_evento,$p_incidencia, $this->mando);
            $no_valida = $variable;
            /****************************************************************************************/
            $mostrar_empleado = "NO";
            if($this->mando == 2){ //si se logeo un gerente
                //verificamos si el emplado esta al mismo nivel organizacional
                //si el empleado pertenece
                $nivel = $this->verifica_nivel_empleado($area_codigo);
                if($nivel == 3) //si el nivel es gerencia entonces tambien debe mostrar
                    $mostrar_empleado = "SI";
            }
            
            if($mostrar == "SI" || $mostrar_empleado == "SI"):
            //if(1):
...
/*DESCRIPCION:  verificamos el flujo de la incidencia para 
                ...
                $ejecuta : 0 = area de apoyo
                           ...
                           2 = gerente 
*/
function verifica_nivel_empleado( $area){
    $cn = $this->getMyConexionADO();
    $params = array($area);
    $sql = "select Area_Nivel_Org from Areas where Area_Codigo = ?";
