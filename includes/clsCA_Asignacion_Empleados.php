...
function listado_evento_log(){
	...
	$ssql.=" else ";
                $ssql.=" case when ca_evento_log.aprobado = 'N' then '' ";
                    ...
                    $ssql.=" end ";
	$ssql.=" end	";
        $ssql.=" ,case ca_evento_log.realizado when 'S' then case when left(cast(ca_eventos.observacion as varchar),5)='CARGA' then 'Carga Masiva' else 'Supervisor' end when 'V' then 'Validador' when 'M' then 'Mando' end  as realizado ";
...
function listado_eventos_validador(){
    ...
    $ssql="";
    $params = array($this->empleado_codigo);
    $consulta = "EXEC SPCA_EVENTOS_LISTADO_MANDO ?";   
    $rs = $cn->Execute($consulta, $params); 
    //echo $ssql;
        $padre=array();
       //$rs = $cn->Execute($ssql);
...
function listado_eventos_validador_apoyo(){
    $rpta="OK";
    $cn = $this->getMyConexionADO();
    $cn->debug=false;
    $ssql="";
    ...
    $ssql.=" inner join areas ar on ar.area_codigo = cas.area_codigo ";
        ...
        //--$ssql.=" and iia.area_codigo = ar.area_codigo ";
    $ssql.=" where ";
        //--$ssql.=" iia.empleado_codigo = ".$this->empleado_codigo."  ";
    
        $ssql.=" 1= ";
            $ssql.=" case when exists(select caia.area_codigo ";
                $ssql.=" from ca_incidencia_areas as caia ";
                $ssql.=" where caia.incidencia_codigo = ci.Incidencia_codigo ";
                    $ssql.=" and caia.empleado_codigo = ".$this->empleado_codigo." and caia.area_codigo = 0) ";
            $ssql.=" then 1 else ";
                $ssql.=" case when exists(select caia.area_codigo ";
                    $ssql.=" from ca_incidencia_areas as caia ";
                    $ssql.=" where caia.incidencia_codigo = ci.Incidencia_codigo ";
                        $ssql.=" and caia.empleado_codigo = ".$this->empleado_codigo." and caia.area_codigo = ar.area_codigo) ";
