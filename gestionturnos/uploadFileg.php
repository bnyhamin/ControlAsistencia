...
$arr_areas=array();
$arr_areas=$gc->obtenerAreasPermitidasController();


//CONTROLAR CARGA SUPLENCIAS
$controlar=FALSE;
$controlarAnalistaMultiArea=FALSE;
if($gc->getVerificaRol($empleado_codigo_registro,3)==1){//EXISTE 3.ROL Administrador
    ...
    $controlar=TRUE;
    $controlarAnalistaMultiArea=FALSE;
}else if($gc->getVerificaRol($empleado_codigo_registro,9)==1){//EXISTE ROL 9. Analista Multiarea
    //$controlar=FALSE;
    $controlarAnalistaMultiArea=TRUE;
    $controlar=TRUE;//cambiar
		...
		for($i=0; $i < $lineas; $i++){ 
			...
			$o->te_fecha_fin="";
                        ...
                        if ($controlar){//solo cargar suplencias de empleados de la misma area del GTR - Suplencia Diaria Area de Usuario
                            ...
                            if($controlarAnalistaMultiArea){
                                if(in_array($emp_area, $arr_areas)){
                                    $o->Addnew_Tmp_Nuevo();
                                    //echo 'hola'.$columna[0];
                                }
                            }else{
