...
require_once("../../Includes/clswfm_disponibilidad_servicio.php");
set_time_limit(30000); 
    ...
    $w = new wfm_disponibilidad_servicio();
    $w->setMyUrl(db_host());
    $w->setMyUser(db_user());
    $w->setMyPwd(db_pass());
...
if(!isset($_POST['Upload'])) { 
	include("subir_archivoServicio.php"); //Si no hemos cargado, mostramos el formulario   
}else{
	...
	if($_FILES['pix']['tmp_name'] == "none") { //Si no se ha cargado o es más grande de lo especificado   
		...
		include("subir_archivoServicio.php"); //mostramos de nuevo el formulario   
		exit();   
	}
	...
	if(!ereg("text/plain",$_FILES['pix']['type'])) { //si no es un archivo de texto   
		...
		include("subir_archivoServicio.php");   
		exit();   
		...
		for($i=0; $i < $lineas ; $i++){ 
			...
			$codigo_campana = $columna[1];
			
			$w->empleado_dni=$empleado_dni;
			$w->codigo_campana=$codigo_campana;
			$w->usuario_registra=$empleado_codigo_registro;
